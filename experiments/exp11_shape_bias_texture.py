...
#   - On the natural context, the shape‑biased ResNet50 is not dramatically more
#     unique than other CNNs.
#   - On the shape‑biased cue‑conflict context, the shape‑biased ResNet50
#     becomes much harder to approximate by convex combinations of peers,
#     i.e. has much higher PIER than standard CNNs.
...
try:
    # If you already moved ImageModelWrapper into a shared module, reuse it.
    from isqed.real_world import ImageModelWrapper  # type: ignore
except Exception:
...
def load_standard_models(device: str) -> Dict[str, ImageModelWrapper]:
    """
    Load a small ImageNet ecosystem of standard models.
    ...
    model_wrappers["ResNet18"] = ImageModelWrapper(res18, "ResNet18", device)

    # Standard ResNet50
    res50 = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V2)
    model_wrappers["ResNet50"] = ImageModelWrapper(res50, "ResNet50", device)
    ...
    model_wrappers["ViT_B16"] = ImageModelWrapper(vit_b16, "ViT_B16", device)

    return model_wrappers


def load_shape_biased_resnet50(device: str) -> ImageModelWrapper:
    """
    Load a shape‑biased ResNet‑50 from Geirhos et al. (Stylized‑ImageNet).

    We directly download the checkpoint using the public Bitbucket URL from
    the official texture‑vs‑shape repository.
    ...
    # Model A from the texture‑vs‑shape repo: trained on Stylized‑ImageNet only.
    url_sin = (
    ...
    checkpoint = load_state_dict_from_url(url_sin, map_location=device)
    ...
    return ImageModelWrapper(backbone, "ShapeResNet50_SIN", device)
...
def run_shape_texture_experiment(
    ...
    print("Loading shape-biased ResNet50 (Stylized-ImageNet)...")
    model_shape = load_shape_biased_resnet50(device=device)

    # Combine into ecosystem
